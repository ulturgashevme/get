...
figure, axes = plt.subplots(figsize = (16, 10), dpi = 800)
...
charge_plot_line   , = axes.plot(charge_data[0], charge_data[1], color = 'green')
discharge_plot_line, = axes.plot(discharge_data[0], discharge_data[1], color = 'red')

charge_plot_line.set_label("Процесс заряда конденсатора в RC-цепочке")
discharge_plot_line.set_label("Процесс разряда конденсатора в RC-цепочке")
...
axes.grid(color = "black", which = "both", linestyle = ':', linewidth = 0.5)

charge_time    = time_arr[volt_max_ind] - time_arr[0]
discharge_time = time_arr[-1] - time_arr[volt_max_ind]

axes.axvline(x = charge_time, ymin=y_limits[0], ymax = volt_max/y_limits[1], color = 'blue', linestyle='dashed')
axes.axhline(y = volt_max, xmin=x_limits[0], xmax = charge_time/x_limits[1], color = 'blue', linestyle='dashed')

axes.scatter(time_arr[volt_max_ind], volt_max, color='blue')

axes.scatter(x = charge_time, y = 0.0, color='blue')
axes.text(x=charge_time+0.1, y = 0.05, s=str(round(charge_time, 2)), fontsize = 12)

axes.scatter(x = 0.0, y = volt_max, color = 'blue')
axes.text(x = 0.1, y = volt_max+0.05, s = str(round(volt_max, 2)), fontsize = 12)

axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Время заряда: " + str(round(charge_time, 2)) + " с"), color = 'green', fontsize = 14)
axes.text(x = (charge_time+discharge_time/2-0.8), y = volt_max/2, s = ("Время разряда: " + str(round(discharge_time, 2)) + " с"), color = 'red', fontsize = 14)
